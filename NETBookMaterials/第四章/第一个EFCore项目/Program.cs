 ...
 * 1、创建实体类
 * 2、实体类配置类 指明实体类对应的表
 * 3、配置DB上下文类 需要继承DbContext，里面成员属性为需要操作的表的实体类  DbContext就相当于数据库，成员属性为数据库中的表
 * 4、需要数据迁移，将数据迁移到数据库 需要两步：Add-Migration Initsecond 暂时还没有传到数据库，需要然后再更新 update-Database 就可以了
